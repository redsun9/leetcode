package leetcode.leetcode37xx.leetcode3751;
...
public class Solution {
    ...
    public static int totalWaviness(int num) {
        int power = 1;
        while (power * 10 <= num) power *= 10;
        return waveness(0, 0, num, power, true, false, new HashMap<>());
    }

    private static int waveness(int digit1, int digit2, int num, int power, boolean samePrefix, boolean hasNonZeroAlready, Map<Key, Integer> cache) {
        int ans = 0;
        if (power == 0) return 0;
        int newPower = power / 10;
    ...
    private record Key(int digit1, int digit2, int power, boolean samePrefix, boolean hasNonZeroAlready) {
    }
}
