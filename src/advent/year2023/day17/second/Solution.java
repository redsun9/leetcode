package advent.year2023.day17.second;
...
public class Solution {
    ...
    public static void main(String[] args) throws IOException {
        try (
                FileInputStream fis = new FileInputStream("src/advent/year2023/day17/second/input.txt");
                Scanner scanner = new Scanner(fis);
                FileOutputStream fos = new FileOutputStream("src/advent/year2023/day17/second/output.txt");
    ...
    private static int solve(int[][] input) {
        ...
        while (!pq.isEmpty()) {
            ...
            for (int k = 1, i1 = i, i2 = i, j1 = j, j2 = j, s1 = s, s2 = s; k <= MAX_MOVEMENT; k++) {
                ...
                if (i1 < m && j1 < n) {
                    s1 += input[i1][j1];
                    if (k >= MIN_MOVEMENT && s1 < dp[i1][j1][1 - direction]) {
                ...
                if (i2 >= 0 && j2 >= 0) {
                    s2 += input[i2][j2];
                    if (k >= MIN_MOVEMENT && s2 < dp[i2][j2][1 - direction]) {
